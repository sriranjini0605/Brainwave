...
const Header = () => {
    ...
    return (
        <div className="fixed top-0 z-50 left-0 w-full border-b border-n-6\
            ...
            <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
                ...
                <nav className={` ${openNavigation ? "flex" : "hidden"}  fixed-top top-[5rem] left-0 right-0 bottom-0 \
                        ...
                        {navigation.map((item) => (
                            <a key={item.id} href={item.url} onClick={handleClick}
                                className={`block relative font-code text-2xl uppercase text-n-1 transition-colors \
                                             hover:text-color-1 ${item.onlyMobile ? "lg:hidden" : ""} px-6 py-6 \
